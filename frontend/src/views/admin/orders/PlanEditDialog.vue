    ...
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div v-if="selectedGroupInfo" class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-gray-500">{{ selectedGroupInfo.custom_window_hours === 5 ? t('payment.admin.limit5Hour') : t('payment.admin.limitHourWindow', { hours: selectedGroupInfo.custom_window_hours ?? '-' }) }}:</span> <span class="ml-1 font-medium text-gray-700 dark:text-gray-300">{{ selectedGroupInfo.custom_limit_usd != null ? '$' + selectedGroupInfo.custom_limit_usd : t('payment.admin.unlimited') }}</span></div>
